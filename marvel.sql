...
select alignment, Count(alignment) as character from Marvels group by alignment;

select name from Marvels Where gender = 'Male' AND hometown = 'USA';

SELECT name, alignment from Marvels group by name having fighting_Skills >= 4;

Select name, gender,
CASE
    WHEN fighting_skills >= 4 Then "ATTACK"
    WHEN energy_Projection >=4 THEN "SUPPORT"
    ELSE "BACKUP"
    END as "position"
from Marvels;

Select Count(*) as team_member,
CASE
    WHEN fighting_skills >= 4 Then "ATTACK"
    WHEN energy_Projection >=4 THEN "SUPPORT"
    ELSE "BACKUP"
...
SELECT gender, 
ROUND (avg(fighting_Skills)) as skill,
ROUND (avg(energy_Projection)) as energy,
...
ROUND (avg(energy_Projection)) as energy,
ROUND (avg(speed)) as speed
from Marvels group by hometown order by skill;
...
ROUND (avg(speed)) as speed,
CASE
    WHEN speed + energy_Projection + fighting_Skills > 16 THEN "SAFE"
    WHEN speed + energy_Projection + fighting_Skills BETWEEN 14 AND 16 THEN "BACKUP"
    ELSE "HELP"
    END as "Safety"
from Marvels group by hometown ;

SELECT 
gender, alignment, count(alignment) as total, Round(avg(fighting_Skills)) as skill
FROM Marvels
